...
class TestGet:
    async def test_get_success(self, client: BitbucketClient, mock_router: respx.MockRouter):
        data = {"key": "PROJ"}
        mock_router.get("/rest/api/1.0/projects/PROJ").mock(return_value=Response(200, json=data))
        result = await client.get("/projects/PROJ")
        assert result == data

    async def test_get_with_params(self, client: BitbucketClient, mock_router: respx.MockRouter):
        data = {"values": []}
        mock_router.get("/rest/api/1.0/projects").mock(return_value=Response(200, json=data))
        result = await client.get("/projects", params={"start": 0, "limit": 10})
        assert result == data

    async def test_get_404_raises(self, client: BitbucketClient, mock_router: respx.MockRouter):
        error_body = {"errors": [{"message": "Project not found", "exceptionName": "NoSuchProjectException"}]}
        mock_router.get("/rest/api/1.0/projects/NOPE").mock(return_value=Response(404, json=error_body))
    ...
    async def test_get_500_non_json(self, client: BitbucketClient, mock_router: respx.MockRouter):
        mock_router.get("/rest/api/1.0/fail").mock(return_value=Response(500, text="Internal Server Error"))
        with pytest.raises(BitbucketAPIError) as exc_info:
...
class TestPost:
    async def test_post_success(self, client: BitbucketClient, mock_router: respx.MockRouter):
        data = {"name": "new-repo"}
        mock_router.post("/rest/api/1.0/projects/PROJ/repos").mock(return_value=Response(201, json=data))
        result = await client.post("/projects/PROJ/repos", json_data={"name": "new-repo"})
        assert result == data


class TestPut:
    async def test_put_success(self, client: BitbucketClient, mock_router: respx.MockRouter):
        data = {"id": 1, "title": "Updated"}
        mock_router.put("/rest/api/1.0/projects/PROJ/repos/repo/pull-requests/1").mock(
            return_value=Response(200, json=data)
        )
        result = await client.put("/projects/PROJ/repos/repo/pull-requests/1", json_data={"title": "Updated"})
        assert result == data
...
class TestGetRaw:
    ...
    async def test_get_raw_error(self, client: BitbucketClient, mock_router: respx.MockRouter):
...
class TestSearch:
    async def test_search_success(self, client: BitbucketClient, mock_router: respx.MockRouter):
        data = {"values": [{"file": "test.py"}]}
        mock_router.get("/rest/search/latest/search").mock(return_value=Response(200, json=data))
        result = await client.search({"query": "hello", "type": "content"})
        assert result == data


class TestGetPaged:
    async def test_injects_pagination_params(self, client: BitbucketClient, mock_router: respx.MockRouter):
        data = {"values": [], "isLastPage": True, "start": 5, "limit": 10, "size": 0}
        route = mock_router.get("/rest/api/1.0/projects").mock(return_value=Response(200, json=data))
...
class TestHandleResponse204:
    async def test_204_returns_empty_dict(self, client: BitbucketClient, mock_router: respx.MockRouter):
